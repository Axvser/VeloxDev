...
| 来源 | `Kind` | 怎么进提示 |
|---|---|---|
| 嵌入资源 | `SkillSourceKind.Embedded`（`EmbeddedSkillSource.cs:47`） | **正文全文注入**，按名字序拼接（`SkillScope.cs:222` `BuildEmbeddedBlock`） |
| 文件系统 | `SkillSourceKind.File`（`FileSkillSource.cs:46`） | **只做广告**：名字 + 描述，包在 `<available_skills>` 的 Agent Skills 约定块里（`SkillScope.cs:283` `BuildAdvertisement`），正文要模型自己用 `load_skill` 取 |

两者**可以同时存在**，`SkillAgentContextProvider.BuildInstructions`（`SkillAgentContextProvider.cs:106-120`）把两个块依次拼起来；**两个都空时返回 `null`**，贡献一个「空标题」不如什么都不贡献（`:101-102`）。

`BuildEmbeddedBlock` 的注释（`SkillScope.cs:217-221`）给了一个可以直接拿来验的等价关系：**全部技能都启用时，它逐字节等于技能可开关之前提示里带的内容**。所以「技能开关坏了」的判据是这句话，不是回忆。
...
- `WithSkills(...)` 之后建的 provider 拿到这个事实（`SkillScope.CreateContextProvider(..., embeddedCorpusDelivered: true)`），于是**不再贡献 `BuildEmbeddedBlock`**，改为贡献 `SkillScope.BuildWithdrawnBlock`（`:251`）：**当前处于停用状态的嵌入技能名单**。
...
- `PromptLanguage` 是**独立维度**：注释明说「语言不是别的地方可观察的」（`:60-63`），虽然 `WithPromptLanguage` 也自增 `Version`（`:67`），provider 仍然按 **`(Version, Language)` 元组**缓存。这是双保险，不是冗余。
...
- `Refresh()` **是宿主动作，不是每轮动作** —— `SkillScope.cs:137-142` 明说：它总是自增 `Version`，在 prompt provider 里每轮调它会直接废掉缓存。
...
`SkillScope.Refresh()`（`:144`）的规则：

1. 某个 source 的 `Discover()` 抛异常 ⇒ **不能拖垮其它 source**。它变成一个名字叫 `{Kind}-source` 的**错误描述符**（`:161-167`）—— 它**没有 `Description`**，所以在 UI 上只会显示成一条错误行。
2. **同名归属**：第一个声明某名字的 source 拥有它；**同一个 source 的多个同名描述符全部保留**（`:171-179`）—— 那是同一个技能的**分语言变体**。这条容易写反：不保留就会丢掉 `zh` 那一份。
3. `Apply(found, owners)` 重建状态，同时**按名字保留启用标志**。

`SetEnabled`（`:191-209`）三个非显然细节：

- **查找发生在编组块内部**（`:199-206`），因为 `Find` 走的是绑定集合 —— 宿主从后台线程切开关时不能在那里读它。
...
2. `new SkillScope().WithSource(src)` 或 `WorkflowAgentScope.WithSkills(SkillScope)`（`WorkflowAgentScope.cs:1391`）。
...
| 捷径 | 为什么错 |
|---|---|
| 把文件技能写成「直接返回全文的 source」 | 会让文件技能也全文注入，绕过了 ad 模式与 `load_skill`；`BuildAdvertisement` 只读 `SkillDescriptor.Name/Description`（`SkillScope.cs:286-292`），你的正文永远到不了提示 |
| 在 prompt provider 里调 `Refresh()` | 每轮自增 `Version`，缓存彻底失效（`SkillScope.cs:137-142`） |
...
| 用 `ConfigureAwait(false)` / 线程池线程切开关 | `Find` 读的是 `ObservableCollection`；`SetEnabled` 的查找被刻意放在编组块里（`SkillScope.cs:199-206`） |
