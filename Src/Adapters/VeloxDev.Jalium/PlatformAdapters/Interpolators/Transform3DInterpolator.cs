...
namespace VeloxDev.Adapters.NativeInterpolators
{
    ...
    public class Transform3DInterpolator : IValueInterpolator
    {
        public List<object?> Interpolate(object? start, object? end, int steps, object? options = null)
        {
            ...
            for (int i = 0; i < steps; i++)
            {
                ...
                result.Add(InterpolateSingle(startT, endT, t));
            }

            result[0] = startT ?? new Transform3DGroup();
            result[steps - 1] = endT ?? new Transform3DGroup();
            return result;
        }
