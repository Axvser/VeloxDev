...
/// <summary>Attached to a node's drag handle: mouse-drag executes node.MoveCommand with deltas
/// relative to a coordinate host (named first, else the first Canvas ancestor).</summary>
public sealed class WorkflowNodeDragBehavior : DependencyObject
{
    ...
    {
        if (sender is not FrameworkElement control || control.GetValue(StateProperty) is not DragState state
            || !state.IsDragging || state.CoordinateHost is null)
        {
            return;
        }
