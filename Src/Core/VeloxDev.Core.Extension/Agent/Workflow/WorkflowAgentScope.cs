...
public class WorkflowAgentScope(IWorkflowTreeViewModel tree) : IAgentToolCallNotifier
{
    ...
    public string ProvideProgressiveContextPrompt(AgentLanguages language)
    {
        ...
        result.AppendLine("## 🚫 Forbidden Operations");
        result.AppendLine();
        result.AppendLine("**Node creation MUST go through CreateNode or CreateAndConfigureNode.** NEVER add nodes by directly modifying the Nodes collection, PatchNodeProperties, or any other means. The Tree's CreateNodeCommand performs essential initialization; bypassing it causes broken state.");
        result.AppendLine();
        ...
        result.AppendLine("- **Node positioning**: CreateNode automatically offsets the position to avoid overlapping existing nodes (30px padding). The response includes actual `x`/`y` and `repositioned=true` if the node was moved. Always check the response for the final position.");
        ...
        result.AppendLine("- **Typed slot properties are auto-created**: Source-generated slot properties (e.g. `InputSlot`, `OutputSlot`) are lazily created on first access — they always exist and never return null. You do NOT need to call CreateSlotOnNode for them. Only use CreateSlotOnNode for dynamically-added slots on nodes that do NOT define typed slot properties.");
        result.AppendLine("- **ResolveSlotId / ConnectByProperty on typed slots**: These tools are safe and efficient — accessing the property triggers auto-creation if needed, so you never get 'slot is null' errors.");
        result.AppendLine();
        result.AppendLine("## ⚠️ Connection Validation (CRITICAL — prevents infinite retry loops)");
        result.AppendLine();
        result.AppendLine("The framework may **silently reject** a connection attempt. All Connect tools now verify and report rejection.");
        result.AppendLine("When you receive `status: \"rejected\"`, **do NOT retry the same connection**. Rejection is deterministic — the same inputs will always be rejected.");
        result.AppendLine();
        result.AppendLine("**Rejection reasons** (from the `reasons` field in the response):");
        result.AppendLine("1. **Channel incompatibility**: The sender slot's channel does not support sending, or the receiver slot's channel does not support receiving. Check `ch` field in GetNodeDetail/ListSlotProperties.");
        result.AppendLine("2. **Same-node connection**: Sender and receiver belong to the same node — always rejected.");
        result.AppendLine("3. **Developer ValidateConnection rule**: The tree's helper has custom validation logic. This is opaque to you. If rejected for this reason, inform the user.");
        result.AppendLine("4. **Channel capacity limit**: One-target/one-source channels may already be at capacity (existing connection occupies the single slot). You may need to disconnect the existing connection first.");
        result.AppendLine();
        result.AppendLine("**Recovery strategy**: Read the `reasons` array → if channel issue, check and possibly change channel with SetSlotChannel → if capacity, disconnect existing → if ValidateConnection, inform user.");
        result.AppendLine();
        result.AppendLine("## ⚡ Operation Ordering Protocol (CRITICAL)");
        result.AppendLine();
        ...
        result.AppendLine("4. **Choose a non-overlapping position**: Before calling CreateNode, check existing node positions (via ListNodes or your cached knowledge) and pick a position that does not overlap existing nodes. Leave at least 30px gap between nodes. The tool will auto-offset if overlap is detected, but you should proactively choose good positions for better layout.");
        result.AppendLine("5. **Call CreateNode or CreateAndConfigureNode** with the resolved type, position, and size. Check the response — if `repositioned=true`, the node was auto-moved to avoid overlap; note the actual `x`/`y` in the response.");
