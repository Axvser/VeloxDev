...
/// no arrowhead. Sized to the host canvas and drawn at the port centers (node.Anchor + geometry) +
/// layout offset (world + ActualOffset — the authoritative model), so it never goes stale on pan. The
/// link comes from DataContext; only real visible links render — the drag-preview VirtualLink is drawn
/// inline by the surface.</summary>
public class TemplateClass : FrameworkElement
{
    ...
    /// preview inline, so the pooled LinkView must skip it. Real links are never this type — even
    /// one whose slots were detached keeps rendering instead of vanishing.</summary>
