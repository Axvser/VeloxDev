...
public partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        // Matches the theme the application starts on; OnThemeChanged keeps it in step afterwards.
    ...
    {
        if (sender is Button { Name: { } name } && int.TryParse(name.AsSpan(4), out var count)) Build(count);
    }
...
public partial class MainWindow
{
    ...
    /// </summary>
    /// <remarks>
    /// This demo themes the window's own background, so the Fluent variant has to follow it: Fluent's control
    /// brushes are built for the surface they sit on, and its dark-variant buttons are translucent white, which
    /// disappears on the white this switch ends on. Flipping the variant when the switch lands keeps the controls
    /// legible at rest in both themes.
    /// </remarks>
    ...
    partial void OnThemeChanged(Type? oldValue, Type? newValue)
    {
        ...
        RequestedThemeVariant = newValue == typeof(Light) ? ThemeVariant.Light : ThemeVariant.Dark;
    }
