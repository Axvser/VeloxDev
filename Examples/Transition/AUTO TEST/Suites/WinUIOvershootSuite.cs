...
/// <summary>
/// The WinUI end of the acceptance run: every scenario the demo's overshoot strip exposes, driven through UI Automation
/// and checked against the theory oracle.
/// </summary>
/// <remarks>
...
public class WinUIOvershootSuite
{
    /// <summary>
    ...
    private static WinUIDemoDriver? _demo;
    ...
    public static void LaunchDemo(TestContext context)
    {
        ...
        if (!AtConfig.Enabled || !AtConfig.RunsOn(WinUIDemoDriver.PlatformName)) return;

        _demo = new WinUIDemoDriver();
        _demo.Launch();
    }
    ...
    [TestMethod]
    public void WinUI_Back_OvershootsAndSettlesExactly() => AssertScalar(ScenarioId.Back);

    /// <summary>A translate on X under <c>Elastic.Out</c>.</summary>
    [TestMethod]
    public void WinUI_Elastic_OvershootsAndSettlesExactly() => AssertScalar(ScenarioId.Elastic);

    /// <summary>A width under <c>Elastic.Out</c>, the second scalar the payload tracks a peak for.</summary>
    [TestMethod]
    public void WinUI_Size_OvershootsAndSettlesExactly() => AssertScalar(ScenarioId.Size);
    ...
    /// </summary>
    [TestMethod]
    public void WinUI_Color_OvershootsOnEveryChannelAndSettlesExactly()
    {
    ...
    /// </summary>
    [TestMethod]
    public void WinUI_Brush_StaysNonSolidThroughoutAndSettles()
    {
        DemoCatalog.RequireEnabled(WinUIDemoDriver.PlatformName);
        var driver = Demo();
    ...
    /// </summary>
    [TestMethod]
    public void WinUI_Elastic_EveryRepeatedRunClearsItsBound()
    {
        DemoCatalog.RequireEnabled(WinUIDemoDriver.PlatformName);
        var driver = Demo();
    ...
    private static WinUIDemoDriver Demo() => _demo
        ?? throw new InvalidOperationException(
            $"{WinUIDemoDriver.PlatformName} was not launched. VELOXDEV_AT is set, so the demo build or its path is the problem.");
...
/// <summary>
/// Proves the thing every other WinUI suite assumes: that the unpackaged demo starts, that its observation surface is
/// reachable through UI Automation, and that its readout is actually ticking.
/// </summary>
/// <remarks>
/// Kept separate from the overshoot suite because it fails for completely different reasons. When this goes red the
/// problem is the launch path, the automation tree, or the payload's spelling; when the overshoot suite goes red the
/// animation is wrong. Sorting a failure into one of those two buckets first is most of the diagnosis.
/// <para>
/// It lives here rather than in <c>LocatorProbeSuite</c> so that adding WinUI touches no file another platform's author
/// owns; the category filter <c>AT.WinUI</c> selects it just the same.
/// </para>
/// </remarks>
[TestClass]
[TestCategory("AT.WinUI")]
public class WinUIProbeSuite
{
    /// <summary>
    /// Drives the WinUI demo's surface: every scenario button present, both readouts present, the payload parseable, and
    /// its sequence number advancing on its own.
    /// </summary>
    [TestMethod]
    public void WinUI_ObservationSurface_IsReachableAndTicking()
    {
        DemoCatalog.RequireEnabled(WinUIDemoDriver.PlatformName);
        using var driver = DemoCatalog.Create(WinUIDemoDriver.PlatformName);
        driver.Launch();
