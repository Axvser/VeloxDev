...
/// <summary>
/// The WPF end of the acceptance run: every scenario the demo's overshoot strip exposes, driven through UI Automation
...
[TestClass]
[TestCategory("AT.WPF")]
public class WpfOvershootSuite
{
    /// <summary>
    ...
    private static WpfDemoDriver? _demo;
    ...
    public static void LaunchDemo(TestContext context)
    {
        ...
        if (!AtConfig.Enabled || !AtConfig.RunsOn(WpfDemoDriver.PlatformName)) return;

        _demo = new WpfDemoDriver();
        _demo.Launch();
    }
    ...
    [TestMethod]
    public void Wpf_Back_OvershootsAndSettlesExactly() => AssertScalar(ScenarioId.Back);

    /// <summary>A translate on X under <c>Elastic.Out</c>.</summary>
    [TestMethod]
    public void Wpf_Elastic_OvershootsAndSettlesExactly() => AssertScalar(ScenarioId.Elastic);

    /// <summary>A width under <c>Elastic.Out</c>, the second scalar the payload tracks a peak for.</summary>
    [TestMethod]
    public void Wpf_Size_OvershootsAndSettlesExactly() => AssertScalar(ScenarioId.Size);
    ...
    /// </summary>
    [TestMethod]
    public void Wpf_Color_OvershootsOnEveryChannelAndSettlesExactly()
    {
    ...
    /// </summary>
    [TestMethod]
    public void Wpf_Brush_StaysNonSolidThroughoutAndSettles()
    {
        DemoCatalog.RequireEnabled(WpfDemoDriver.PlatformName);
        var driver = Demo();
    ...
    /// </summary>
    [TestMethod]
    public void Wpf_Elastic_EveryRepeatedRunClearsItsBound()
    {
        DemoCatalog.RequireEnabled(WpfDemoDriver.PlatformName);
        var driver = Demo();
    ...
    private static WpfDemoDriver Demo() => _demo
        ?? throw new InvalidOperationException(
            $"{WpfDemoDriver.PlatformName} was not launched. VELOXDEV_AT is set, so the demo build or its path is the problem.");
