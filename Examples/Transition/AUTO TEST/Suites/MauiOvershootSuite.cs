...
/// <summary>
/// The MAUI end of the acceptance run: every scenario the Windows head of the demo's overshoot strip exposes, driven
/// through UI Automation and checked against the theory oracle.
/// </summary>
/// <remarks>
/// <para>
/// This suite matters out of proportion to the demo's size, because of which branch of the library it covers. A MAUI
/// <c>Transition&lt;T&gt;</c> fills the priority type parameter with <c>NonPriority</c> — MAUI has no
/// <c>DispatcherPriority</c> — so every run here exercises the priority-free sampling path, not the priority-carrying
/// one the WPF demo drives. The two paths share the curves but not their call sites, which is exactly why the same
/// curves are re-checked here rather than assumed to follow from WPF passing.
/// </para>
/// <para>
...
public class MauiOvershootSuite
{
    /// <summary>
    ...
    private static MauiDemoDriver? _demo;
    ...
    public static void LaunchDemo(TestContext context)
    {
        ...
        if (!AtConfig.Enabled || !AtConfig.RunsOn(MauiDemoDriver.PlatformName)) return;

        _demo = new MauiDemoDriver();
        _demo.Launch();
    }
    ...
    [TestMethod]
    public void Maui_Back_OvershootsAndSettlesExactly() => AssertScalar(ScenarioId.Back);

    /// <summary>A translate on X under <c>Elastic.Out</c>.</summary>
    [TestMethod]
    public void Maui_Elastic_OvershootsAndSettlesExactly() => AssertScalar(ScenarioId.Elastic);

    /// <summary>
    /// A width under <c>Elastic.Out</c>. The demo animates <c>WidthRequest</c> rather than a <c>Size</c>: a MAUI
    /// <c>VisualElement</c> has no writable size-typed property, so this is the nearest surface the framework offers and
    /// the one the demo settled on.
    /// </summary>
    [TestMethod]
    public void Maui_Size_OvershootsAndSettlesExactly() => AssertScalar(ScenarioId.Size);
    ...
    /// </summary>
    [TestMethod]
    public void Maui_Color_OvershootsOnEveryChannelAndSettlesExactly()
    {
    ...
    /// </summary>
    [TestMethod]
    public void Maui_Brush_StaysNonSolidThroughoutAndSettles()
    {
        DemoCatalog.RequireEnabled(MauiDemoDriver.PlatformName);
        var driver = Demo();
    ...
    /// </summary>
    [TestMethod]
    public void Maui_Elastic_EveryRepeatedRunClearsItsBound()
    {
        DemoCatalog.RequireEnabled(MauiDemoDriver.PlatformName);
        var driver = Demo();
    ...
    private static MauiDemoDriver Demo() => _demo
        ?? throw new InvalidOperationException(
            $"{MauiDemoDriver.PlatformName} was not launched. VELOXDEV_AT is set, so the demo build or its path is the problem.");
...
/// <summary>
/// Proves the thing the overshoot suite assumes: that the MAUI demo starts, that its observation surface is reachable
/// through UI Automation on Windows, and that its readout is actually ticking.
/// </summary>
/// <remarks>
/// Kept as its own class because it fails for completely different reasons. When this goes red the problem is the
/// launch path, the automation tree, or the payload's spelling; when the overshoot suite goes red the animation is
/// wrong. Sorting a failure into one of those two buckets first is most of the diagnosis.
/// </remarks>
[TestClass]
[TestCategory("AT.MAUI")]
public class MauiObservationSurfaceSuite
{
    /// <summary>
    /// Drives the MAUI demo's surface: every scenario button present, both readouts present, the payload parseable, and
    /// its sequence number advancing on its own.
    /// </summary>
    [TestMethod]
    public void Maui_ObservationSurface_IsReachableAndTicking()
    {
        DemoCatalog.RequireEnabled(MauiDemoDriver.PlatformName);
        using var driver = DemoCatalog.Create(MauiDemoDriver.PlatformName);
        driver.Launch();
