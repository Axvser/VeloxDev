...
internal static class SamplerProbe
{
    ...
    internal static class Kinds
    {
        ...
        /// </summary>
        /// <remarks>
        /// 名字不再取自采样器类型名 —— 两条用的是同一个 <c>ColorSampler</c>，区分它们的是路径里那个下标。
        /// 相邻下标必须是两条不同的路径：索引器的 <c>PropertyInfo</c> 对每个下标都是同一个 "Item"，
    ...
    [
        ...
        new(Kinds.GradientStop0Color,
            ...
            null, () => new ColorSampler(),
            () => BrushStartColor, () => BrushEndColor,
            Path: () => GradientStopPath(0)),
        new(Kinds.GradientStop1Color,
            "索引器路径：相邻下标必须互不覆盖（Ramp.GradientStops[1].Color）—— 与上一行同一个采样器、同一刻并行跑。",
            null, () => new ColorSampler(),
            () => TintStart, () => TintEnd,
            Path: () => GradientStopPath(1)),
    ];

    /// <summary>
    /// 建一条 <c>Ramp.GradientStops[i].Color</c>。下标是常量，所以这条路径的身份只由下标决定。
    /// </summary>
    /// <remarks>
    /// 用的是 <see cref="SamplerSubject.Ramp"/> 而不是 Fill：那一位是 null 开头、由每一行自己的起点值装填的，
    /// 而要写的元素必须在写它之前就先存在。Ramp 在构造时建好，所以这条路径一上来就能解析。
    /// </remarks>
    ...
    /// <remarks>
    /// 每条只建<b>一个</b>实例。路径是状态字典的键，每次新建一个虽然按值相等，但索引实参来自闭包时
    /// 未必相等；缓存下来就没有这层疑问。
    /// </remarks>
