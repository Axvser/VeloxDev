...
/// 关系，那些留在窗口那侧（与 WPF / WinForms 版同一分工）。
/// </remarks>
...
/// 走到批量路径上就变成了"后一条把前一条掐掉"，只剩最后一条在跑。
/// </param>
...
/// </para>
/// <para>
/// Jalium 的子元素被父级裁掉（<c>ClipToBounds</c>），跑到台子外的元素在这里不是"溢出"，是"没了"。
/// </para>
/// </remarks>
internal sealed class SamplerBench
{
    ...
    /// <summary>采样器那一格的尺寸。标尺（<see cref="SamplerSubject.Scale"/>）是按它定出来的，不能随手改。</summary>
    internal const double SamplerStageWidth = 96d;
    internal const double SamplerStageHeight = 50d;
    ...
    {
        // 采样器那一格比别的行矮（标尺是按 50 高定出来的），所以在等高的元素区里居中放一层自己那块格子，
    ...
    /// WPF 式 Name→AutomationId 回退，两条路都通。
    /// </summary>
    internal static void SetToken(FrameworkElement element, string automationId)
    {
    ...
    private static Canvas Center(FrameworkElement content)
    {
    ...
    private static FrameworkElement BuildRow(CaseRow row)
    {
        ...
        caption.Inlines.Add(new Run(row.Title) { FontWeight = FontWeights.Bold });
        caption.Inlines.Add(new Run($"  {row.Description}"));
    ...
    private static Button MakeControl(string text, string automationId, Action action, int column)
    {
        ...
        // 一排人够不着的行会绿着过去）。聚焦会经 RequestBringIntoView 落到列表的 ScrollViewer 上，所以
        // 这里不需要再补一次 —— 真实滚动由"点得到"这件事自己证明。
