...
{
    /// <summary>
    /// Standalone animation test for the VeloxDev.WinUI PlatformAdapters (TransitionSystem).
    /// </summary>
    /// <remarks>
    /// 版面与其余几个平台的 Transition demo 对齐（见 <c>MainWindow.xaml</c>）：顶栏（全局三件 + 五种加载方式
    ...
    public sealed partial class MainWindow : Window
    {
        ...
        /// </summary>
        /// <remarks>
        /// 必须存进字段：只放在局部变量里，C#/WinRT 的 CCW 会被 GC 回收，Tick 随之停掉，观测面在读了几十拍之后
        /// 就不再前进（验收套件会把这看成"功能坏了"）。两条演出用的定时器同理。
        /// </remarks>
        ...
        public MainWindow()
        {
            ...
            // 旋转绕元素自己的中心：行里的台子是裁边的，绕左上角转 180° 会把方块整个甩到台子外，
            // 而"跑出去看不见"与"没在跑"在屏幕上分不开 —— 这正是每一行都要装得下自己那段行程的意思。
            // 列表里的元素只有这一个带旋转（Rec2 那条走的是投影，旋转中心由它自己的 CenterOfRotation 给）。
            Rec1.RenderTransformOrigin = new Point(0.5d, 0.5d);
            ...
            ((FrameworkElement)Content).Loaded += (s, e) =>
            {
                ...
                btnReset.Click += (s, e) => ResetAllCases();

                // 读数用定时器采样目标属性，而不是订阅 effect 的事件 —— 流水线每段都会 Clone() effect，
                // 在这里订阅的处理函数不是真正触发的那个。
                _readoutTimer = DispatcherQueue.GetForCurrentThread()!.CreateTimer();
                ...
                // 默认圆角），都不是采样器声明的起点，于是"偏离起点"的行数在什么都没跑的时候就是满的 ——
                // 顶栏那个数会变成一句假命题。
                ResetSamplerRows();
            };
        }
        ...
        private void RebuildCaseList()
        {
            ...
            _caseRows.Add(LoadRow("combine", "平移 + 投影",
                "整块投影被换掉：平面投影的九个字段逐项外推，第二段还会转去动颜色。",
        ...
        {
            Fill = CreateRec0Brush(), Width = 80, Height = 60, RenderTransform = CreateRec0Transform(),
        };

        private readonly Rectangle Rec1 = new() { Fill = new SolidColorBrush(Colors.Lime), Width = 80, Height = 60 };
        ...
        {
            Fill = new SolidColorBrush(ColorStart), Width = 80, Height = 60, RenderTransform = new TranslateTransform(),
        };
        ...
        {
            Fill = new SolidColorBrush(ColorStart), Width = 80, Height = 60,
        };

        private readonly Rectangle Over2 = new()
        {
            Fill = new SolidColorBrush(Color.FromArgb(0xFF, 0x7A, 0x5A, 0xA5)), Width = 80, Height = 60,
        };
        ...
        /// </remarks>
        private static readonly Color ColorStart = Color.FromArgb(0xFF, 0x3A, 0x6E, 0xA5);

        private static readonly Color ColorTarget = Color.FromArgb(0xFF, 0x80, 0x80, 0xD0);
        ...
        // WinUI 的约束：Projection 与 RenderTransform(Scale) 互斥，所以先把 Projection 清掉再写其余的。
        private void RestoreTranslateRow()
        {
            Rec0.Projection = null;
            Rec0.RenderTransform = null;
        ...
        private void RestoreColorStart() => Over1.Fill = new SolidColorBrush(ColorStart);

        private void RestoreSizeStart() => Over2.Width = WidthStart;
        ...
        private DispatcherQueueTimer? _benchTimer;
        ...
        {
            var back = (Over0.RenderTransform as TranslateTransform)?.X ?? 0d;
            var elastic = (Over4.RenderTransform as TranslateTransform)?.X ?? 0d;
        ...
        private string BuildState()
        {
            ...
            var x = (Over0.RenderTransform as TranslateTransform)?.X ?? 0d;
            var elasticX = (Over4.RenderTransform as TranslateTransform)?.X ?? 0d;
        ...
        /// （<c>null</c> 画刷、灰底色、默认圆角），不是采样器声明的起点，于是"全部启动后 &gt; 0"在什么都没跑时
    ...
    public sealed partial class MainWindow
    {
        ...
        // 跑出行外会被裁掉 —— 而"跑出去看不见"与"没在跑"在屏幕上分不开。验收侧只钉静止态（CreateRec0Reset/1/2），
        // 行程是观感，不是契约。
        // -----------------------------------------------------------------------------------------------
        ...
        // Delayed animation: reverse rotation. 旋转绕元素自己的中心（见构造函数里那句 RenderTransformOrigin）——
        // 行里的台子是裁边的，绕左上角转 180° 会把方块整个甩到台子外。
        ...
        // Combined animation: projection rotation + colour change.
        // 这一条写的是**投影**而不是 RenderTransform：投影的旋转中心由它自己的 CenterOfRotation 给（0.5/0.5，
        // 也就是元素中心），所以它不需要 RenderTransformOrigin，也不会被行列的位移算到 TranslateX 里去。
