...
internal static class SamplerProbe
{
    ...
    [
        ...
        new(Kinds.GridLengthSampler, "栅格长度：两端单位不同时保持起点 —— 单位换算没有插值的意义，所以这条是离散的。", nameof(SamplerSubject.Length), () => new GridLengthSampler(),
    ...
    internal static Measurement Measure(object? value) => value switch
    {
        ...
        GridLength length => new("GridLength", [length.Value, (int)length.GridUnitType]),
        PlaneProjection projection
            => new("PlaneProjection",
                [projection.RotationX, projection.RotationY, projection.RotationZ,
                 projection.CenterOfRotationX, projection.CenterOfRotationY, projection.CenterOfRotationZ,
                 projection.GlobalOffsetX, projection.GlobalOffsetY, projection.GlobalOffsetZ]),
