...
/// </summary>
public sealed class MinimapOverlay : Panel, IWorkflowMinimapOverlay, IWorkflowMinimapScrollSource
{
    private readonly Color _background = ParseColor("#D2141922");
    private readonly Color _border = ParseColor("#DC94A3B8");
    private readonly Color _nodeFill = ParseColor("#DC38BDF8");
    private readonly Color _viewportStroke = ParseColor("#F0FFFFFF");
    ...
    {
        // The pointer is the target viewport center; matching WPF/Avalonia/WinUI/MAUI/Razor,
        // pressing anywhere recenters the view. No content clamp: the surface grows instead,
        // so the block can be dragged to the minimap edge and pan the surface into empty space.
        double cx = x;
        double cy = y;
    ...
    /// <summary>
    /// Content bounds + scale shared by painting and drag hit-mapping. Returns null
    /// when there are no nodes yet, so the minimap stays blank until the tree lays out.
    /// </summary>
