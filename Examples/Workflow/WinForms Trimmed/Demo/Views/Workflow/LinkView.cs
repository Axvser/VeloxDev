...
public sealed class LinkView : Control
{
    ...
    /// The host canvas writes all slot anchors back first, then translates the origin and
    /// calls this for every link in world coordinates — links no longer live as overlapping
    /// full-size transparent child windows, which WinForms clips (WS_CLIPSIBLINGS) so only
    /// the topmost ever paints. As a standalone control OnPaint uses the same path.
    /// </summary>
