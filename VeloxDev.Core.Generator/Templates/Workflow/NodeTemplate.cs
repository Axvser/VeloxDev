...
{
    public static string FromTaskConfig(int semaphore)
    {
        string command = (semaphore > 1) ? "ConcurrentVeloxCommand" : "VeloxCommand";
        string param3 = (semaphore > 1) ? $",semaphore: {semaphore}" : string.Empty;
        return $$"""
                     ...
                     public global::VeloxDev.Core.Interfaces.MVVM.IVeloxCommand WorkCommand
                     {
                         get
                         {
                             _buffer_WorkCommand ??= new global::VeloxDev.Core.MVVM.{{command}}(
                                 executeAsync: WorkTask,canExecute: _ => true{{param3}});
