...
| 异步 | `await tree.SerializeAsync()` | 异步序列化 |
| 异步 | `await json.DeserializeAsync<T>()` | 异步反序列化 |
| 格式化 | `new SerializationOptions().WithIndented()` | 缩进输出 |
| 字节 | `tree.SerializeToUtf8Bytes()` | 序列化为 UTF8 字节 |
| 流 | `tree.SerializeToStreamAsync(stream)` | 序列化到流 |
...
sequenceDiagram
    ...
    participant JSON as JSON 文档

    App->>Ser: tree.Serialize()
    Ser->>Tree: 遍历 Nodes[]
    Ser->>Tree: 遍历 Links[]
    Ser->>Tree: 读取 Layout
    Ser->>Tree: 读取 VirtualLink
    Ser->>JSON: 构建 JSON 对象
    JSON-->>App: 返回 JSON 字符串

    Note over App,JSON: 反序列化

    App->>Ser: json.Deserialize()
    Ser->>JSON: 解析 JSON
    Ser->>Ser: 还原 Tree / Nodes / Links
    Ser->>Ser: 恢复 Slot 引用关系
    Ser-->>App: 返回完整 Tree
```
