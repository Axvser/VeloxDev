...
{
    [Generator(LanguageNames.CSharp)] //数据驱动型
    public class ViewModelModule : IIncrementalGenerator
    {
        ...
        private static void GenerateSource(SourceProductionContext context, (Compilation Compilation, ImmutableArray<ClassDeclarationSyntax> Classes) input)
        {
            ...
            foreach (var roslyn in uniqueTargets.Select(t => new ViewModelRoslyn(t.Value, t.Key, compilation)).Where(r => r.IsViewModel))
            {
                var fullName = AnalizeHelper.GetViewModelFileName(roslyn.Symbol, roslyn.Syntax);
